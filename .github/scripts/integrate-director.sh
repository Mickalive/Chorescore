...
integrate_role() {
  ...
  local enabled candidate_dir audit_dir report decision metadata has_delta
  ...
  jq -e --arg cycle "$cycle" --arg role "$role" '
    ...
    (.hasDelta|type)=="boolean" and (.verificationOnly|type)=="boolean" and
  ...
  ' "$metadata" >/dev/null
  has_delta=$(jq -r '.hasDelta' "$metadata")
  ...
  if [[ "$decision" == accept ]]; then
    if [[ "$has_delta" == true ]]; then
      ...
      test ! -s "$candidate_dir/candidate.patch"
      set_role_manifest "$role" "verified" "$decision" "independent audit accepted the existing source; no product delta required"
    fi
  else
...
if ! git diff --cached --quiet; then
  git commit -m "ChoreScore factory $cycle: independently audited product evidence"
...
OPENCODE_RETRY_LABEL=director \
  ...
  "You are ChoreScore Release Director for factory cycle $cycle. Read MAIN_PROMPT.md, governance/roles/RELEASE_DIRECTOR.md, governance/RELEASE_DEFINITION.json, directives/DIRECTOR.md, docs/RELEASE_STATUS.json, directives/TASKS.json, docs/NEXT_CYCLE.md and all current audit reports. Trusted integration manifest: $manifest. Product code and audit reports have already been persisted by the trusted shell. Never edit product code, docs/security, workflows, governance, agents, dependencies or lockfiles. Update only directives/TASKS.json, directives/MOBILE.md, directives/BACKEND.md, directives/AUDITOR.md, docs/NEXT_CYCLE.md, docs/RELEASE_STATUS.json and reports/director/RUN_${cycle}.md + JSON. Preserve every completed criterion and its evidence. For repair/reject audits, make the requiredFix the first bounded task for that role. An accepted role with manifest status 'verified' is fresh independent evidence that the current accepted source already satisfies its bounded objective without a product delta; use that evidence to resolve stale findings and complete the assigned criterion when the release definition is satisfied. DRC-05 completes only after its unresolved release-blocking accessibility findings are resolved with tests and accepted audit evidence. DRC-07 completes only after accurate security/public documentation is present with accepted audit evidence. When DRC-01 through DRC-05 and DRC-07 are all complete and no unresolved mustFixBeforeRelease finding remains, do not create another source-readiness lane: set DRC-06 status to in_progress, pendingArtifact to 'DRC-06', activeCriteria to [], disable both code roles, and set report decision='stop' so the trusted shell builds the APK immediately. Never mark DRC-06 complete yourself; only the trusted release shell can do that after artifact and runtime-smoke evidence. Otherwise keep at least one useful local task active and report decision='continue'. Stagnation, provider failure, absent candidate or negative audit never means final completion."
...
if ! jq -e --arg cycle "$cycle" '
  .schemaVersion==1 and (.cycle|tostring)==$cycle and
  (.decision=="continue" or .decision=="stop") and
  (.reason|type=="string" and length>0) and
  (.progressEvidence|type=="array")
' "$report" >/dev/null; then
  echo "::error::Director report contract is invalid." >&2
  exit 21
fi

if ! jq -e -n --slurpfile before "$RUNNER_TEMP/release-before-director.json" --slurpfile after docs/RELEASE_STATUS.json '
  [$before[0].criteria[] | select(.status=="complete")] as $done |
  all($done[];
    . as $old |
    any($after[0].criteria[];
      .id==$old.id and .status=="complete" and
      ((.evidence // [])|length) >= (($old.evidence // [])|length)
    )
  )
' >/dev/null; then
  echo "::error::Director regressed a completed release criterion or its evidence." >&2
  exit 22
fi
...
if ! jq -e '
  ...
  ((.criteria | map(select(.id=="DRC-06")) | length)==1) and
  ((.criteria | map(select(.id=="DRC-06")))[0].status != "complete")
' docs/RELEASE_STATUS.json >/dev/null; then
  echo "::error::Release state contract is invalid before artifact attestation." >&2
  exit 23
fi
...
if [[ "$pending" == true ]]; then
  if ! jq -e '
    all(.criteria[]; if .id=="DRC-06" then .status=="in_progress" else .status=="complete" end) and
    (.activeCriteria|length)==0 and
    all(.openFindings[]?; (.mustFixBeforeRelease!=true) or .status=="resolved")
  ' docs/RELEASE_STATUS.json >/dev/null; then
    echo "::error::DRC-06 artifact handoff is not release-ready." >&2
    exit 24
  fi
