...
while :; do
  ...
  deleted_any=false
  while IFS=$'\t' read -r run_id old_number status; do [[ -n "$run_id" ]] || continue; if [[ "$status" != completed ]]; then gh api -X POST "repos/$repo/actions/runs/$run_id/force-cancel" >/dev/null 2>&1 || gh api -X POST "repos/$repo/actions/runs/$run_id/cancel" >/dev/null 2>&1 || true; fi; if gh api -X DELETE "repos/$repo/actions/runs/$run_id" >/dev/null 2>&1; then echo "Deleted obsolete workflow run $run_id (#$old_number, $status)."; deleted_any=true; else echo "::warning::GitHub retained obsolete workflow run $run_id (#$old_number, $status)."; fi; done < <(jq -r '.[]|[.id,.run_number,.status]|@tsv' <<<"$old_runs")
