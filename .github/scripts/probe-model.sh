...
nonce="CHORESCORE_PROBE_${GITHUB_RUN_ID:-local}_${RANDOM}_${RANDOM}_$(date +%s)"
printf '%s\n' "$nonce" > "$target"
log="$out/probe.log"
started=$(date +%s)
...
if [[ "$healthy" != true ]]; then
  if (( status == 124 || status == 137 )); then
    ...
    reason="rate-limit"
  elif grep -Eqi 'forbidden|403|not supported|unavailable' "$log"; then
    reason="unavailable"
  elif grep -Eqi 'unexpected server error|internal server error|upstream|service unavailable|502|503|504' "$log"; then
