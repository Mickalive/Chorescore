...
/**
 * Identité attendue par les décisions d'invitation. Alimentée par le câblage
 * avec `observedCaller(request)` (module observedCaller.ts) : chaque porte
 * ci-dessous reflète une propriété constatée sur la requête brute, jamais une
 * constante supposée acquise par les gardes amont. Si `requireCaller` venait
 * à s'affaiblir, ces portes restent exécutables et refusent avec les codes et
 * messages historiques (constat F1-identite-decorative-cablage).
 */
